...
print(f"开始采集 {len(dates)} 个交易日数据", flush=True)
print("="*60, flush=True)
...
for i, date in enumerate(dates):
    ...
    if (i + 1) % 10 == 0 or i == len(dates) - 1:
        ...
        print(f"[{i+1}/{len(dates)}] {date} 当天{daily_count}条 累计{total_records}条 用时{elapsed/60:.1f}分", flush=True)
    
    time.sleep(0.05)

elapsed = time.time() - start_time
print("="*60, flush=True)
print(f"完成! 共 {total_records} 条记录, 用时 {elapsed/60:.1f} 分钟", flush=True)

# 统计
cursor.execute('SELECT source, limit_type, COUNT(*) FROM limit_up_down GROUP BY source, limit_type')
print("\n数据分布:", flush=True)
for row in cursor.fetchall():
    print(f"  {row[0]} - {row[1]}: {row[2]} 条", flush=True)

cursor.execute('SELECT COUNT(DISTINCT date) FROM limit_up_down')
days = cursor.fetchone()[0]
print(f"\n覆盖交易日: {days} 天", flush=True)
